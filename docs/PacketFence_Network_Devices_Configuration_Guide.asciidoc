...
NOTE: While using the WebAuth mode on the Meraki controller with a version superior to X.X, you need to use "Role mapping by Switch Role" in the switch configuration. More details below.

In the version X.X of the Meraki firmware, the WebAuth mode has been improved and now allows CoA and the use of redirect URL via a RADIUS attribute.

The configuration for your SSID should look like this:

image::docs/images/Meraki-config-1.png[scaledwidth="100%",alt="Security and RADIUS configuration"]

image::docs/images/Meraki-config-2.png[scaledwidth="100%",alt="Network configuration"]

NOTE: It is mandatory that you use the Airespace-ACL-Name as "RADIUS attribute specifying group policy name".

When done you will need to add the address for the redirection in the switch configuration in PacketFence. Add the Meraki IP to PacketFence, then choose the module "Meraki cloud controller v2" and add the portal interface ip in the "Portal URL" section. It should look like the following:

image::docs/images/Meraki-config-portal.png[scaledwidth="100%",alt="Portal URL"] #TO ADD

Next, configure the roles for the devices on your network. Go in `Network-wide->Group policies`, then you will be able to create policies that can be configured in the roles in the switch configuration in PacketFence. Creation of the policy `Guest devices`:

image::docs/images/Meraki-config-policy.png[scaledwidth="100%",alt="Group policies"]

Role configuration in PacketFence:

image::docs/images/Meraki-config-role.png[scaledwidth="100%",alt="Role configuration"]
