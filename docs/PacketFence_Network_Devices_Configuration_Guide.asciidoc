...
In the version X.X of the Meraki firmware, the WebAuth mode has been improved and now allow CoA, and the use of redirect-URL instead of passing by the external captive portal.
...
While done you will need to add the address for the redirection in the switch configuration in PacketFence. Add the Meraki IP to PacketFence, then choose the module "Meraki cloud controller v2" and add the portal interface ip in the "Portal URL" section. It should look like the following:

----
 http://192.168.1.5
----

The switch Role you will enter, will be the one you created as a group policy in the Meraki configration. To do so, go through `Network-wide->Group policies`, here you can create policies that will be applied to the devices which register through this Meraki network.

Also make sure to select RADIUS as the deauthentication method.
