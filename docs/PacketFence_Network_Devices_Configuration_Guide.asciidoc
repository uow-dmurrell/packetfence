...
If you have the version X.X the configuration for your SSID should look like this:
...
While done you will need to add the address of where to send the redirection in the switch configuration in PacketFence. Add the Meraki IP to PacketFence, then choose the module "Meraki cloud controller v2" and add the portal interface ip in the "Portal URL" section. It should look like the following:

----
 http://192.168.1.5
----

Also make sure to select RADIUS as the deauthentication merhod.
